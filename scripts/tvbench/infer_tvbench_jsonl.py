...
def parse_multi_choice_response(response, all_choices, index2ans):
    """
    Match NextQA/TVBench multiple-choice answer extraction logic.
    """
    for char in [",", ".", "!", "?", ";", ":", "'"]:
...
def summarize(results):
    """Summarize accuracy overall and broken down by TVBench category."""
    ...
    for item in results:
        ...
        bucket = by_category.setdefault(cat, {"total": 0, "correct": 0, "accuracy": 0.0})
        bucket["total"] += 1
...
def main():
    parser = argparse.ArgumentParser(description="Run VideoLLaMA3 inference on TVBench SFT JSONL files.")
    parser.add_argument("--model-path", required=True)
    parser.add_argument("--jsonl-path", default="data/tvbench/mini_sft.jsonl")
    parser.add_argument("--data-folder", default="/home/hmkang/project/videollama3/TVBench/video")
    ...
    with output_path.open("w", encoding="utf-8") as f:
        for record in tqdm(records, desc="TVBench JSONL inference"):
