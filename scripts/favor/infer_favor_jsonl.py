...
def summarize(results):
    """Summarize accuracy overall and broken down by FAVOR task_type."""
    ...
    for item in results:
        if item.get("correct") is None:
            continue
        tt = str(item.get("task_type", "unknown"))
        bucket = by_task_type.setdefault(tt, {"total": 0, "correct": 0, "accuracy": 0.0})
        bucket["total"] += 1
...
def main():
    parser = argparse.ArgumentParser(description="Run VideoLLaMA3 inference on FAVOR SFT JSONL files.")
    parser.add_argument("--model-path", required=True)
    parser.add_argument("--jsonl-path", default="data/favor/mini_sft.jsonl")
    parser.add_argument("--data-folder", default="/home/hmkang/project/videollama3/FAVOR/videos")
    ...
    with output_path.open("w", encoding="utf-8") as f:
        for record in tqdm(records, desc="FAVOR JSONL inference"):
            human_prompt = next(
                message["value"] for message in record["conversations"] if message["from"] == "human"
            )
            ...
            result = {
                ...
                "question_key": record.get("metadata", {}).get("question_key", ""),
                "gt": gt,
                "pred": None,
                "response": None,
                "correct": None,
