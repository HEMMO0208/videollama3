#!/usr/bin/env bash
# Submit FAVOR eval jobs sweeping over MAX_FRAMES values.
#
# Usage:
#   ./scripts/favor/submit_eval_sweep.sh <model_kind> <max_frames...> [-- <sbatch_args...>]
#
# Examples:
#   ./scripts/favor/submit_eval_sweep.sh baseline 100
#   ./scripts/favor/submit_eval_sweep.sh diffusion 8 16 32 64 100
#   ./scripts/favor/submit_eval_sweep.sh causal_diffusion 8 16 32 64 100
#   ./scripts/favor/submit_eval_sweep.sh all 8 16 32 64 100
#   ./scripts/favor/submit_eval_sweep.sh all 8 16 32 64 100 -- --dependency=afterok:12345
#   ./scripts/favor/submit_eval_sweep.sh baseline 32 64 -- --dependency=afterany:111,222 --mail-type=END
#
...
for kind in "${KINDS[@]}"; do
    for mf in "${MAX_FRAMES_LIST[@]}"; do
        ...
        sbatch \
            ...
            "${EXTRA_SBATCH_ARGS[@]}" \
            "$SCRIPT_DIR/eval_favor_jsonl_${kind}.sbatch"
    done
done
