...
class Videollama3MetaForCausalLM(ABC):
    ...
    def _maybe_truncate_visual_tokens(
        ...
        visual_segment_ids: Optional[torch.LongTensor] = None,
        visual_token_indices: Optional[torch.LongTensor] = None,
    ):
        if position_ids is None or mm_features.shape[0] == input_ids.eq(self.config.image_token_index).sum():
            if visual_segment_ids is None:
                return mm_features, compression_mask
            return mm_features, compression_mask, visual_segment_ids, visual_token_indices
        ...
        if visual_token_indices is not None:
            visual_token_indices = visual_token_indices[truncation_mask]
        return (
            mm_features[truncation_mask],
            compression_mask[truncation_mask],
            visual_segment_ids[truncation_mask],
            visual_token_indices,
        )

    def _get_diffusion_layout(
        self,
        batched_num_patches: torch.LongTensor,
        grid_sizes: torch.LongTensor,
        merge_sizes: torch.LongTensor,
        ...
        segment_ids = []
        token_indices = []
        full_segment_lengths = []
        chunk_size = int(getattr(self.config, "diffusion_chunk_size", 4))
        for num_patches, grid_size, merge_size, modal in zip(batched_num_patches, grid_sizes, merge_sizes, modals):
            num_patches = int(num_patches.item())
            if modal != "video":
                segment_ids.append(torch.full((num_patches,), -1, dtype=torch.long, device=grid_sizes.device))
                token_indices.append(torch.full((num_patches,), -1, dtype=torch.long, device=grid_sizes.device))
                full_segment_lengths.append([])
                continue
            t, h, w = [int(x) for x in grid_size.tolist()]
            merge = int(merge_size.item()) if torch.is_tensor(merge_size) else int(merge_size)
            tokens_per_frame = max(1, (h // merge) * (w // merge))
            frame_ids = torch.arange(t, device=grid_sizes.device, dtype=torch.long) // chunk_size
            cur_segment_ids = frame_ids.repeat_interleave(tokens_per_frame)
            local_positions = []
            lengths = []
            num_segments = int(cur_segment_ids.max().item()) + 1 if cur_segment_ids.numel() else 0
            for segment_id in range(num_segments):
                seg_len = int((cur_segment_ids == segment_id).sum().item())
                lengths.append(seg_len)
                local_positions.append(torch.arange(seg_len, device=grid_sizes.device, dtype=torch.long))
            cur_token_indices = torch.cat(local_positions, dim=0) if local_positions else torch.empty((0,), dtype=torch.long, device=grid_sizes.device)
            segment_ids.append(cur_segment_ids[:num_patches])
            token_indices.append(cur_token_indices[:num_patches])
            full_segment_lengths.append(lengths)
        if not segment_ids:
            empty = torch.empty((0,), dtype=torch.long, device=grid_sizes.device)
            return empty, empty, []
        return torch.cat(segment_ids, dim=0), torch.cat(token_indices, dim=0), full_segment_lengths
    ...
    def _compress_visual_tokens(
        ...
        if visual_segment_ids is not None:
            visual_segment_ids = visual_segment_ids[compression_mask]
        if visual_token_indices is not None:
            visual_token_indices = visual_token_indices[compression_mask]
        ...
        return mm_features, input_ids, attention_mask, position_ids, labels, visual_segment_ids, visual_token_indices
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        visual_segment_ids, visual_token_indices, full_segment_lengths = self._get_diffusion_layout(
            batched_num_patches, grid_sizes, merge_sizes, modals
        )
        visual_segment_ids = visual_segment_ids.to(input_ids.device)
        visual_token_indices = visual_token_indices.to(input_ids.device)

        compression_mask = self._get_compression_mask(
            pixel_values, batched_num_patches, grid_sizes, merge_sizes, modals
        )
        mm_features, compression_mask, visual_segment_ids, visual_token_indices = self._maybe_truncate_visual_tokens(
            mm_features,
            compression_mask,
            batched_num_patches,
            modals,
        ...
        if self.config.use_token_compression:
            ...
            mm_features, input_ids, attention_mask, position_ids, labels, visual_segment_ids, visual_token_indices = self._compress_visual_tokens(
                compression_mask,
                mm_features,
                input_ids,
                attention_mask,
        ...
        condition_mask = image_selected.clone()
        condition_segment_ids = torch.full_like(input_ids, -1, dtype=torch.long)
        condition_token_indices = torch.full_like(input_ids, -1, dtype=torch.long)
        if visual_segment_ids.numel() == int(image_selected.sum().item()):
            condition_segment_ids[image_selected] = visual_segment_ids.to(condition_segment_ids.device)
            condition_token_indices[image_selected] = visual_token_indices.to(condition_token_indices.device)
            condition_mask = condition_mask & condition_segment_ids.ge(0)

        # 6. reshape back to batched format
        C = inputs_embeds.shape[-1]
        inputs_embeds = inputs_embeds.reshape(B, -1, C)
        self._diffusion_condition_mask = condition_mask.reshape(B, -1)
        self._diffusion_condition_segment_ids = condition_segment_ids.reshape(B, -1)
        self._diffusion_condition_token_indices = condition_token_indices.reshape(B, -1)
        self._diffusion_full_segment_lengths = full_segment_lengths
        if attention_mask is not None:
