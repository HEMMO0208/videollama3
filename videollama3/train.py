...
from videollama3.diffusion import (
    MaskedVideoTokenDiffusion,
    RossVAE,
    chunk_frames_for_diffusion,
    diffusion_target_geometry,
    pad_square_resize_frame,
    prepare_vae_frame,
)
from videollama3.diffusion_trainer import VideoLLaMA3DiffusionTrainer
...
class ModelArguments:
    ...
    diffusion_enable: bool = field(default=False)
    diffusion_hidden_size: int = field(default=1024)
    ...
    def _convert_normal(self, data_dict):
        ...
        if 'image' in data_dict and data_dict['image'] is not None:
            ...
            if isinstance(video_file, list) and len(video_file) == 1:
                ...
                if getattr(self.data_args, "diffusion_enable", False):
                    image_size = int(getattr(self.data_args, "diffusion_vae_image_size", 384))
                    siglip_size = int(getattr(self.data_args, "diffusion_siglip_frame_size", 336))
                    chunk_size = int(getattr(self.data_args, "diffusion_chunk_size", 4))
                    if len(images) % chunk_size != 0:
                        pad_count = chunk_size - (len(images) % chunk_size)
                        images = list(images) + [images[-1] for _ in range(pad_count)]
                        if len(timestamps) > 0:
                            timestamps = list(timestamps) + [timestamps[-1] for _ in range(pad_count)]
        ...
        return modal, images, messages, merge_size, diffusion_images
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            if "stream" in data_dict and data_dict["stream"]:
                ...
                modal, images, messages, merge_size, diffusion_images = self._convert_normal(data_dict)
            ...
            if getattr(self.data_args, "diffusion_enable", False):
                query_prob = min(max(float(getattr(self.data_args, "diffusion_query_prob", 1.0)), 0.0), 1.0)
                data_dict["diffusion_images"] = diffusion_images
                data_dict["diffusion_chunk_masks"] = [
                    bool(torch.rand((), dtype=torch.float32).item() < query_prob)
                    for _ in diffusion_images
                ]
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if any("diffusion_images" in x for x in instances):
            batch["diffusion_images"] = [x.get("diffusion_images", []) for x in instances]
            batch["diffusion_chunk_masks"] = [x.get("diffusion_chunk_masks", []) for x in instances]
...
def train(attn_implementation=None):
    ...
    data_args.diffusion_enable = model_args.diffusion_enable
    data_args.diffusion_vae_image_size = model_args.diffusion_vae_image_size
    data_args.diffusion_chunk_size = model_args.diffusion_chunk_size

    local_rank = training_args.local_rank
    if model_args.diffusion_enable and data_args.use_batch_flattening:
        rank0_print("Disabling batch flattening for diffusion supervision to preserve sample/chunk boundaries.")
        data_args.use_batch_flattening = False
    if model_args.diffusion_enable and model_args.use_token_compression and training_args.per_device_train_batch_size != 1:
        raise ValueError(
    ...
    if model_args.vision_encoder is not None:
        ...
        if model_args.diffusion_enable:
            patch_size = vision_encoder.image_processor.patch_size
            data_args.diffusion_siglip_frame_size = (
                model_args.diffusion_target_spatial * data_args.video_merge_size * patch_size
            )
        ...
        model.config.diffusion_vae_image_size = model_args.diffusion_vae_image_size
        model.config.diffusion_target_spatial = model_args.diffusion_target_spatial
    ...
    # select a Trainer
    if model_args.diffusion_enable:
        if model_args.mm_pixel_decoder is None:
            raise ValueError("--mm_pixel_decoder is required when --diffusion_enable true")
        vae = RossVAE(model_args.mm_pixel_decoder)
        geometry = diffusion_target_geometry(
            model_args.diffusion_vae_image_size,
            model_args.diffusion_target_spatial,
            vae.latent_channels,
            model_args.diffusion_chunk_size,
        )
        model.config.diffusion_pixel_unshuffle = geometry.unshuffle_factor
        model.config.diffusion_max_tokens = geometry.tokens_per_chunk
        diffusion_head = MaskedVideoTokenDiffusion(
            token_dim=geometry.token_dim,
            hidden_size=model.config.hidden_size,
            depth=model_args.diffusion_depth,
            max_latent_tokens=geometry.tokens_per_chunk,
