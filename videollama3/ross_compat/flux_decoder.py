...
class FluxDecoder(nn.Module):
    ...
    def scaling_factor(self):
        scaling_factor = getattr(self.pixel_decoder.config, "scaling_factor", None)
        return 1.0 if scaling_factor is None else scaling_factor

    @property
    def shift_factor(self):
        shift_factor = getattr(self.pixel_decoder.config, "shift_factor", None)
        return 0.0 if shift_factor is None else shift_factor
